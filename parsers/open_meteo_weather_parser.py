...
class WeatherParser:
    ...
    def get_city_weather(self, lat: float, lon: float):
        ...
        # Process hourly data. The order of variables needs
    ...
    def get_weather(self):
        ...
        for i, city in enumerate(data):
            ...
            if os.path.isfile(f'weather_by_city/{self.startdate}_'
            ...
            while j < 3:
                try:
                    ...
                    self.save_to_csv(city_weather, f'weather_by_city/'
                                     f'{self.startdate}_{self.enddate}_'
                                     f'{city_id}.csv')
                    break
                except Exception as e:
                    ...
                    if delay == 'Minutely':
                        ...
                        print(f'{datetime.now().strftime('%d.%m.%Y %H:%M:%S')}'
                              f': Exceeded {delay}, waiting {h} hours {m} '
                              f'minutes {s} seconds')
                        time.sleep(timeout)
                    else:
                        print(e)
            ...
            print(f'{datetime.now().strftime('%d.%m.%Y %H:%M:%S')}: Attempt: '
                  ...
                  f'{city['id']} rows: {(len(city_weather['date'])
                                         if len(city_weather) != 0 else 0)}')
